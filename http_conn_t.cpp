...
*/
const char* doc_root = "/home/jiezai_cbd/projects/vscode_linux/ThreadPoolHTTPServer";
        ...
        switch ( m_check_state )
        {
            ...
            default:
            {
                ...
                */
                ...
                strncpy( m_real_file + len, "/500.html", FILENAME_LEN - len - 1 );
                printf("the request file change to:%s\n",m_real_file);

                int fd = open( "./500.html", O_RDONLY );
                stat("./500.html",&m_file_stat);
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if ( stat( m_real_file, &m_file_stat ) < 0 )
    {
        ...
        */
        ...
        strcpy( m_real_file, doc_root );
        int len = strlen( doc_root );
        strncpy( m_real_file + len, "/404.html", FILENAME_LEN - len - 1 );
        printf("the request file change to:%s\n",m_real_file);

        int fd = open( "./404.html", O_RDONLY );
        stat("./404.html",&m_file_stat);
        m_file_address = ( char* )mmap( 0, m_file_stat.st_size, PROT_READ, MAP_PRIVATE, fd, 0 );
        close( fd );
        /*=============wangweijie 2020/12/19===============*/
        return NO_RESOURCE;
    ...
    if ( ! ( m_file_stat.st_mode & S_IROTH ) )
    {
        ...
        */
        ...
        strcpy( m_real_file, doc_root );
        int len = strlen( doc_root );
        strncpy( m_real_file + len, "/403.html", FILENAME_LEN - len - 1 );
        printf("the request file change to:%s\n",m_real_file);

        int fd = open( "./403.html", O_RDONLY );
        stat("./403.html",&m_file_stat);
        m_file_address = ( char* )mmap( 0, m_file_stat.st_size, PROT_READ, MAP_PRIVATE, fd, 0 );
        close( fd );
        /*=============wangweijie 2020/12/19===============*/
    ...
    if ( S_ISDIR( m_file_stat.st_mode ) )
    {
        ...
        */
        ...
        strcpy( m_real_file, doc_root );
        int len = strlen( doc_root );
        strncpy( m_real_file + len, "/400.html", FILENAME_LEN - len - 1 );
        printf("the request file change to:%s\n",m_real_file);

        int fd = open( "./400.html", O_RDONLY );
        stat("./400.html",&m_file_stat);
        m_file_address = ( char* )mmap( 0, m_file_stat.st_size, PROT_READ, MAP_PRIVATE, fd, 0 );
        close( fd );
        /*=============wangweijie 2020/12/19===============*/
        return BAD_REQUEST;
...
{
    printf("线程调用process()，开始HTTP处理\n");
    printf("线程调用process_read()，开始HTTP请求处理\n");
    ...
    printf("线程调用process_write()，开始HTTP请求回送内容处理\n");
    bool write_ret = process_write( read_ret );
    if ( ! write_ret )
    {
        /*process_write后的close_conn*/
        printf("process_write后的close_conn\n");
        close_conn();
    }
    /*process_write后的设置EPOLLOUT*/
    printf("process_write后的设置EPOLLOUT\n");
    modfd( m_epollfd, m_sockfd, EPOLLOUT );
    printf("process()函数返回\n");
}
